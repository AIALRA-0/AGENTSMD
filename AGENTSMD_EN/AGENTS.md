...
* Manual tampering with index history lines is prohibited；Indexes are maintained by scripts，Can't help butAGENTmaintenance。
...
3. Execute information acquisition according to workflow，Execution and post-execution observations，and update entries as needed
...
* One-click closed loop：`scripts/md_sync.sh [--scope <DEPT>]`
...
### Workflow：Stability reinforcement（non-attack）
...
### Workflow：Post-release monitoring and corrections
...
### Workflow：Post-event review and knowledge accumulation
...
### Workflow：Cross-department overhaul（Multiple directory reconstruction）
...
* One-click entrance：`bash scripts/md_sync.sh`
...
* `DECISIONMD` Used to document architecture-level decisions（Architecture Decision Records, ADR），Including major technical routes、structural design、Specification changes、Core trade-offs, etc.
* Update correction mode（update）
...
* Before regression testing or major changes，Need to trace back the relevant historical decision-making chain，Confirm that this action does not violate existing architectural constraints
...
* After an architecture-level decision occurs and consensus is reached，Must add DECISION Version
...
* Modification or overwriting of history is prohibited DECISION File，To maintain the historical authenticity of decision-making
...
* It is prohibited to write decision-making content that is inconsistent with the actual consensus or implementation.
...
* `ERRORMD` Record system updates and compilation phases（Non-operation and maintenance）error、Warning and repair closed loop。
* log mode（log）。
...
* The goal is to make Agent Quickly retrieve and reuse confirmed knowledge before coding and decision-making，Avoid duplication of research and understanding costs。
...
* `SECURITYMD` Not ordinary bug Log，Not documenting day-to-day advice or preventive discussions that did not occur。
...
* entry mode（entry）。
* The goal is to make every development、Repair、Releases can all be executed based on unified test specifications，avoid“Changed but didn’t test clearly”。
...
* When the same task involves multiple modules，Corresponding test items need to be read separately，Not read-only for a single baseline。
...
* Disable recording of unexecutable pseudo-commands or pathless tools。
