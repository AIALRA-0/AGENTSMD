...
1. 读取 `AGENTS.md`。
2. 每轮工作流开始前，必须先执行 Workflow 清理预检：
   * 检查当前变更集中是否存在历史遗留 `RUN_INFO_WORKFLOW_*` 轨迹文件。
   * 保证“本轮任务”仅有一个 workflow trace 候选；若发现多个，先清理（提交/归档/重命名）再继续。
3. 读取目标工作流，并按模式选择读取目标部门
4. 按照工作流执行信息获取，执行和执行后观察，并按需更新条目
5. 如果需要更新写入，写入前读取 `REGISTRYMD` 最新条目检查保护路径，若命中受保护路径，停止写入并请求外部确认。
6. 同一任务内可连续完成多个部门修改；全部改完后仅执行一次全量 `scripts/md_sync.sh`。
7. 若自动修复失败，输出失败清单并提醒外部用户。
...
* 每轮任务在选择 workflow_id 后、正式写入前，必须通过“Workflow 清理预检”；预检未通过不得进入执行阶段。
* 单轮任务只允许一个 `RUN_INFO_WORKFLOW_*` 作为主 trace；禁止同一变更集并行保留多个 workflow trace。
